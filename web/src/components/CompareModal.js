    ...
    return `
      <div style="flex: 1; min-width: 200px; background: var(--bg-surface-2); border-radius: var(--radius-md); padding: 1rem; display: flex; flex-direction: column; gap: 0.75rem;">
        ...
        <div style="font-size: 0.8125rem; display: flex; flex-direction: column; gap: 0.4rem;">
          <div><strong>⚡ Work Commute:</strong> <span style="color: #34d399;">${escapeHtml(commute)}</span></div>
          <div><strong>🛡️ Superfund:</strong> <span style="color: #f87171;">${escapeHtml(sfDist)} mi</span></div>
          <div><strong>📐 Layout:</strong> ${escapeHtml(item.bedrooms)}bd / ${escapeHtml(item.bathrooms)}ba (${item.sqft ? `${escapeHtml(item.sqft)} sf` : 'N/A'})</div>
          <div><strong>🧺 Laundry:</strong> ${escapeHtml(item.amenities?.laundry || 'N/A')}</div>
          <div><strong>❄️ Cooling:</strong> ${escapeHtml(item.amenities?.cooling || 'None')}</div>
          <div><strong>🚗 Parking:</strong> ${escapeHtml(item.amenities?.parking || 'Unspecified')}</div>
          <div><strong>🐾 Pets:</strong> ${item.pets?.allowed ? `Yes (${escapeHtml(item.pets?.monthly_fee || 'fees apply')})` : 'No'}</div>
        </div>

        <div style="border-top: 1px solid var(--border-subtle); padding-top: 0.5rem; font-size: 0.75rem;">
          <div><strong>Rating:</strong> ⭐ ${escapeHtml(ann.rating || 'Unrated')}</div>
          <div style="margin-top: 2px;"><strong>Status:</strong> ${escapeHtml(ann.visit_status || 'Unvisited')}</div>
          ${ann.highlights ? `<div style="margin-top: 4px; color: #34d399;"><strong>Pros:</strong> ${escapeHtml(ann.highlights)}</div>` : ''}
          ${ann.lowlights ? `<div style="margin-top: 2px; color: #f87171;"><strong>Cons:</strong> ${escapeHtml(ann.lowlights)}</div>` : ''}
        </div>
      </div>
    `;
  }).join('');
