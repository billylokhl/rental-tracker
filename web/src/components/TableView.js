...
export function renderTableView(container, listings, annotations, comparedIds, onRowClick, onCompareToggle, onHideToggle) {
  ...
  let rowsHtml = listings.map(item => {
    ...
    if (item.crime_safety && item.crime_safety.overall_safety_grade) {
      const color = isSafeGrade(item.crime_safety.overall_safety_grade) ? '#34d399' : '#f87171';
      safetyHtml = `<span style="color: ${color}; font-weight: bold;" title="Property: ${escapeHtml(item.crime_safety.property_grade)} | Violent: ${escapeHtml(item.crime_safety.violent_grade)}">${escapeHtml(item.crime_safety.overall_safety_grade)}</span>`;
    }

    const bedBath = `${escapeHtml(item.bedrooms)}bd / ${escapeHtml(item.bathrooms)}ba`;
    ...
    return `
      <tr class="table-row" data-id="${item.id}" style="cursor: pointer;">
        ...
        <td><strong>${escapeHtml(ann.rating || '-')}</strong></td>
        <td>
          <div style="display: flex; align-items: center; gap: 0.4rem; flex-wrap: wrap;">
            <div style="font-weight: 600; color: var(--text-main);">${escapeHtml(item.title)}</div>
            ${item.unit_number ? `<span style="font-size: 10px; background: rgba(2,132,199,0.2); color: #38bdf8; padding: 1px 5px; border-radius: 3px; font-weight: 700;">${escapeHtml(formatUnitBadge(item.unit_number))}</span>` : ''}
            <a href="${escapeHtml(listingUrl)}" target="_blank" rel="noopener noreferrer" title="Open listing" onclick="event.stopPropagation();" style="color: #38bdf8; font-size: 11px; text-decoration: underline;">
              Listing ↗
            </a>
            ${firstMediaUrl ? `
              <a href="${escapeHtml(firstMediaUrl)}" target="_blank" rel="noopener noreferrer" title="View tour album" onclick="event.stopPropagation();" style="color: #34d399; font-size: 11px; text-decoration: underline; font-weight: 700;">
                📸 Media (${mediaUrls.length}) ↗
              </a>
            ` : ''}
          </div>
          <div style="font-size: 11px; color: var(--text-dim);">${item.street_address ? `${escapeHtml(item.street_address)}, ` : ''}${escapeHtml(item.city)}, ${escapeHtml(item.zip)}</div>
        </td>
        <td style="font-family: var(--font-mono); font-weight: 700; color: #38bdf8;">${escapeHtml(item.rent_display)}</td>
        <td>${bedBath}</td>
        <td style="font-family: var(--font-mono);">${item.sqft ? `${escapeHtml(item.sqft)} sf` : '-'}</td>
        <td style="font-weight: 600; color: #fbbf24;">${escapeHtml(avail)}</td>
        <td style="font-weight: 600; color: #34d399;">${escapeHtml(commute)}</td>
        <td>${safetyHtml}</td>
        <td style="color: ${typeof sfDist === 'number' && sfDist < 1.0 ? '#f87171' : 'inherit'};">${escapeHtml(sfDist)} mi</td>
        <td>${escapeHtml(parking)}</td>
        <td>${escapeHtml(appFee)}</td>
        <td>${escapeHtml(item.amenities?.laundry || '-')}</td>
        <td>${item.pets?.allowed ? 'Yes' : 'No'}</td>
        <td style="text-transform: capitalize;">${item.status === 'off-market' ? '<span style="color: #f87171; font-weight: 700;">🛑 Off-Market</span>' : escapeHtml(ann.visit_status || 'unvisited')}</td>
        <td onclick="event.stopPropagation();">
